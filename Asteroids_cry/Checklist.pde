...
When the ship has a collision, make the ship invulnerable to collisions for a short period of time (such as 60 frames). The ship’s show function should also draw the ship differently during this time to indicate this invulnerable state.
...
Fire should have a vel that is opposite of the ship’s direction and come out of the ship. Particles should have a random vel and come out of the center of the destroyed object.
...
Task F: Reset
Make sure the game resets when you return from GAMEOVER to INTRO. You should reset all ship variables (loc, vel, direction, and lives) and the amount of Asteroids.
